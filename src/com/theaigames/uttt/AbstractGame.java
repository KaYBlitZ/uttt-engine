...
public abstract class AbstractGame implements GameLogic {
	...
	public void setupEngine(String args[]) throws IOException, RuntimeException {
		...
		if(DEV_MODE) {
			...
			for(int i = 0; i < NUM_TEST_BOTS; i++) {
				this.engine.addPlayer(TEST_BOT, "ID_" + String.valueOf(i));
			}
		...
		List<String> botArgs = new ArrayList<String>();
		...
		for(int i=0; i < args.length; i++) {
			botIds.add("ID_" + String.valueOf(i));
			System.out.println(args[i]);
			botArgs.add(args[i]);
		}
		
		// check if the starting arguments are passed correctly
		if (botIds.isEmpty() || botArgs.isEmpty() || botIds.size() != botArgs.size())
			throw new RuntimeException("Missing some arguments.");
		
		// add the players
		for (int i = 0; i < botIds.size(); i++) {
			this.engine.addPlayer(botArgs.get(i), botIds.get(i));
		}
	}
	...
	{
		for(PlayerBot ioPlayer : this.engine.getPlayers())
